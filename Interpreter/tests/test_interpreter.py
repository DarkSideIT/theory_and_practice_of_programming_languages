...
class TestInterpreter:
    ...
    def test_add(self, interpreter):
        assert interpreter.eval("3+2") == 5

    def test_sub(self, interpreter):
        assert interpreter.eval("2-4") == -2

    def test_mul(self, interpreter):
        assert interpreter.eval("25*4") == 100

    assert interpreter.eval("15+2*2") == 19

    def test_div(self, interpreter):
        assert interpreter.eval("6/2") == 3
